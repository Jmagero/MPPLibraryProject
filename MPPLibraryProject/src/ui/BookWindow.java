...
public class BookWindow extends JFrame implements SystemWindow{

	public static final BookWindow UI = new BookWindow();
	private static final long serialVersionUID = 1L;
	private boolean isInitialized = false;

	private BookUseCase addBookUseCase = ControllerFactory.createAddBookUseCase();
	...
	private JScrollPane initializeTable() {
		...
		// load books
		List<Book> bookData = searchBookUseCase.getBookCollection();
	...
	private void attachAddBookButtonListener(JButton butn) {
		butn.addActionListener((evt) -> {
			...
			try {
				...
				// load books
				List<domain.Book> data = searchBookUseCase.getBookCollection();
				...
				for (domain.Book bk : data) {
					String strISBN = bk.getISBN();
					String strTitle = bk.getTitle();

					for (BookCopy bc : bk.getBookcopies()) {
			...
			for (domain.Book lm : dataa) {
				String isbn = lm.getISBN();
				String title = lm.getTitle();
				for (BookCopy bc : lm.getBookcopies()) {
					jtmodel.addRow(new Object[] { isbn, bc.getCopyNum(), title, bc.isAvailable() });
				}
